...
create table public.seasons (
  ...
  constraint seasons_transfer_wait_days_valid check (transfer_wait_days >= 0)
);
...
create table public.payments (
  ...
  constraint payments_target_consistency check (
    (payment_type = 'membership' and player_id is not null)
    or (payment_type = 'team_fee' and team_id is not null)
    or (payment_type = 'other' and (player_id is not null or team_id is not null))
  )
);
...
create unique index teams_season_slug_active_uidx
  on public.teams (season_id, lower(slug))
  where deleted_at is null;

create unique index teams_season_name_active_uidx
...
create unique index team_memberships_active_team_captain_uidx
  on public.team_memberships (season_id, team_id)
...
create index team_memberships_team_id_idx on public.team_memberships (team_id) where deleted_at is null;
create index team_memberships_player_id_idx on public.team_memberships (player_id) where deleted_at is null;
create index team_memberships_season_team_idx on public.team_memberships (season_id, team_id) where deleted_at is null;
...
create index transfers_from_team_id_idx on public.transfers (from_team_id) where deleted_at is null;
create index transfers_to_team_id_idx on public.transfers (to_team_id) where deleted_at is null;
...
create index payments_team_id_idx on public.payments (team_id) where deleted_at is null;
...
create or replace function public.is_team_captain(check_team_id uuid, check_season_id uuid)
returns boolean
language sql
stable
security definer
...
create or replace function public.is_team_member(check_team_id uuid, check_season_id uuid)
returns boolean
language sql
stable
security definer
...
begin
  ...
  if new.status = 'requested' and new.effective_on > season_transfer_deadline then
    raise exception 'Transfers are allowed only until the season transfer deadline';
  end if;

  if new.effective_on < (new.requested_at::date + season_transfer_wait_days) then
    raise exception 'Transfer effective date must respect season transfer waiting period';
  end if;
  ...
  if exists (
    ...
    raise exception 'Transfer teams must belong to the same season as the transfer';
  end if;
...
create policy "Captains can manage team player memberships"
  ...
  using (
    public.is_team_captain(team_id, season_id)
...
create policy "Players can view own transfers"
  ...
  using (
    ...
    and (
      ...
      or public.is_team_member(from_team_id, season_id)
      or public.is_team_member(to_team_id, season_id)
...
create policy "Captains can request transfers to own team"
  ...
  with check (
    deleted_at is null
    and status = 'requested'
    and public.is_team_captain(to_team_id, season_id)
  );
...
create policy "Players can view own payments"
  ...
  using (
    ...
    and (
      player_id = public.current_player_id()
      or public.is_team_captain(team_id, season_id)
      or public.has_role(array['moderator', 'admin']::public.app_role[])
    )
  );
