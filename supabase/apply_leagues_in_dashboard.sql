-- Run this once in Supabase Dashboard > SQL Editor if /admin/leagues says
-- that league tables are missing.
...
drop trigger if exists league_groups_set_updated_at on public.league_groups;
create trigger league_groups_set_updated_at
  before update on public.league_groups
  for each row execute function public.set_updated_at();

drop trigger if exists league_group_teams_set_updated_at on public.league_group_teams;
create trigger league_group_teams_set_updated_at
  before update on public.league_group_teams
  for each row execute function public.set_updated_at();

drop trigger if exists leagues_audit on public.leagues;
create trigger leagues_audit
  after insert or update or delete on public.leagues
  for each row execute function public.audit_row_change();

drop trigger if exists league_groups_audit on public.league_groups;
create trigger league_groups_audit
  after insert or update or delete on public.league_groups
  for each row execute function public.audit_row_change();

drop trigger if exists league_group_teams_audit on public.league_group_teams;
...
drop policy if exists "Anyone can view active leagues" on public.leagues;
create policy "Anyone can view active leagues"
  on public.leagues for select
  using (deleted_at is null);

drop policy if exists "Anyone can view active league groups" on public.league_groups;
create policy "Anyone can view active league groups"
  on public.league_groups for select
  using (deleted_at is null);

drop policy if exists "Anyone can view active league group teams" on public.league_group_teams;
create policy "Anyone can view active league group teams"
  on public.league_group_teams for select
  using (deleted_at is null);

drop policy if exists "Moderators can manage leagues" on public.leagues;
...
drop policy if exists "Moderators can manage league groups" on public.league_groups;
create policy "Moderators can manage league groups"
  on public.league_groups for all
  using (public.has_role(array['moderator', 'admin']::public.app_role[]))
  with check (public.has_role(array['moderator', 'admin']::public.app_role[]));

drop policy if exists "Moderators can manage league group teams" on public.league_group_teams;
