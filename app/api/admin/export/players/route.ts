...
function csvValue(value: string | boolean | null | undefined) {
  const text = value == null ? "" : String(value);
  return `"${text.replace(/"/g, '""')}"`;
}

function csvRow(values: Array<string | boolean | null | undefined>) {
...
export async function GET(request: Request) {
  ...
  const [playersResult, seasonsResult, teamsResult, teamSeasonsResult, membershipsResult] = await Promise.all([
    ...
    supabase
      ...
      .select("player_id, season_id, team_season_id, left_on")
      .is("deleted_at", null)
      .is("left_on", null)
